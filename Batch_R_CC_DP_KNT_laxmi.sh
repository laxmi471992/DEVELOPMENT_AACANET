...
# LK05052026 add this function to check if data exists for a given query, returning true if it does and false if it doesn't. 

check_data_exists() {
    local query="$1"

    db2 -x "$query" 2>/dev/null | grep -q 1
    return $?
}
# LK05052026 end of function to check if data exists for a given query.
...
run_stage() {
    ...
    #  LK05052026 comment this run job for court cost and add a check to see if there is data for court cost for the given bill date before running it. If there is no data, log that information and skip running the court cost script for that bill date.
    # run_job "Court Cost" "$COURT_SCRIPT" "$stage_billdate"


    COURT_QUERY="SELECT 1 FROM RMAACABHS WHERE BILLDATE='$stage_billdate' AND RMSTRANCDE='1A' FETCH FIRST 1 ROW ONLY"

    

    if check_data_exists "$COURT_QUERY"; then
        run_job "Court Cost" "$COURT_SCRIPT" "$stage_billdate"
    else
        log "No Court Cost data found for $stage_billdate. Skipping..."
    fi

    REMIT_QUERY="SELECT 1 FROM RMAACABHS 
    WHERE BILLDATE='$stage_billdate' 
    AND RMSTRANCDE >= '50' 
    AND RMSTRANCDE <= '59'
    AND RMSTRANCDE <> '51'
    AND BLAAINNM LIKE '%R%'"

    if check_data_exists "$REMIT_QUERY"; then
    run_job "Remittance" "$REMIT_SCRIPT" "$stage_billdate"
    else
        log "No Remittance data found for $stage_billdate. Skipping..."
    fi

    # run_job "Remittance" "$REMIT_SCRIPT" "$stage_billdate"


        DIRECT_QUERY="SELECT 1 FROM RMAACABHS 
    WHERE BILLDATE='$stage_billdate' 
    AND RMSTRANCDE='51'"
    ...
    if check_data_exists "$DIRECT_QUERY"; then
    run_job "Direct Pay" "$DIRECT_SCRIPT" "$stage_billdate"
    else
        log "No Direct Pay data found for $stage_billdate. Skipping..."
    fi

   #  LK05052026 end all scripts 
