		...
		if ($results1['numRows'] > 0) {
            ...
            // ============================================================
                ...
                //Laxmi developed:at least one record exists
					...
					foreach ($results2['results'] as $resultRow) {

						$PaymentAmount += (float)$resultRow['Payment Amount'];
						$PaymentAmountwise += (float)$resultRow['Payment Amount'];

						$RemitAmount += (float)$resultRow['Remit Amount'];
						$RemitAmountwise += (float)$resultRow['Remit Amount'];

						$FeeRequestedByFirm += (float)$resultRow['Fee Requested By Firm'];
						$FeeRequestedByFirmwise += (float)$resultRow['Fee Requested By Firm'];

						$FeePaidToFirm += (float)$resultRow['Fee Paid To Firm'];
						$FeePaidToFirmwise += (float)$resultRow['Fee Paid To Firm'];

						$AmountPaidToFirm += (float)$resultRow['Amount Paid To Firm'];
